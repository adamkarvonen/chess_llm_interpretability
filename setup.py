...
# This takes a 'filtered_games.csv' file and creates the following files:
# - board_seqs_string.csv (basically a copy of filtered_games.csv but with the header removed
# and an assert statement to ensure that all rows are of the same length)
# - board_seqs_int.npy board_seqs_string.csv but with the strings converted to integer sequences using the meta.pkl file
# - dots_indices.npy a NumPy array of indices of every '.' in the string. This will hopefully provide a reasonable starting point for training a linear probe.
...
df = pd.read_csv(f"{DATA_DIR}filtered_games.csv")
...
df["transcript"].to_csv(f"{DATA_DIR}board_seqs_string.csv", index=False, header=False)
...
np.save(f"{DATA_DIR}board_seqs_int.npy", board_seqs_int)
...
np.save(f"{DATA_DIR}dots_indices.npy", dots_indices)
